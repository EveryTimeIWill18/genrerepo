    ...
    def to_pandas_DataFrame(cls):
        ...
        try:
            if cls.CONTAINER is not None:
                # convert dictionary data to a pandas DataFrame
                s1 = pd.Series(list(cls.CONTAINER.keys()), index=list(cls.CONTAINER.keys()))
                s2 = pd.Series(cls.CONTAINER.values(), index=list(cls.CONTAINER.keys()))
                cls.DATAFRAME = s1.to_frame(name='File_Names').join(s2.to_frame(name='Text_Data'))
            else:
            ...
            try:
                ...
                while  self.data_container.get_page() < pdf_readr.getNumPages():
                    ...
                    try:
                        ...
                        # remove punctuation from text (added: 12/21/2018)
                        cleaned_pdf_str = cleaned_pdf_str.translate((None, string.punctuation))

                        # remove spaces larger than one (added: 12/21/2018)
                        cleaned_pdf_str = ' '.join(cleaned_pdf_str.split())
                ...
                print("Malformed PDF file, skipping...")
        ...
        try:
            if os.path.exists(pickle_path):
                ...
                try:
                    ...
                    pickle.dump(self.data_container.to_pandas_DataFrame())
...
def main():
    ...
    filepath = "N:\\USD\\Business Data and Analytics\\Will dev folder\\compressed_pdf_files"
    test_source_path = 'N:\\USD\\Business Data and Analytics\\Will dev folder\\SA_Claims_DIR\\doc_files'

    extract_pdf = ParsePdf(f_path=test_source_path)
    extract_pdf.extract_text(chunk_size=len(extract_pdf.pdf_files))

    raw_data = extract_pdf.data_container.CONTAINER
    extract_pdf.data_container.to_pandas_DataFrame()
    doc_to_pdf_df = extract_pdf.data_container.DATAFRAME
    pprint(type(doc_to_pdf_df))
    pprint(doc_to_pdf_df.info)

    pickle_path = 'N:\\USD\\Business Data and Analytics\\Will dev folder\\pickle_path'
    save_ = 'doc_df' + str(datetime.today())[:10] + '.pickle'
    output = open(save_, 'wb')
    pickle.dump(doc_to_pdf_df, output)
    output.close()
